...
在 Java 1.5 发行版本之前，我们使用 List 定义了一个只保存正方形对象的列表 squareList。如果不小心将一个长方形对象放进了列表中，这一错误插入照样编译和运行并且不会出现任何错误，直到我们从列表中获取求面积的时候才会受到错误提示：
```java
// 目标是定义一个只保存正方形的列表
...
```
通过这条声明，编译器直到 squareList 应该只包含 Square 实例，并会给予保证，准确的告诉我们哪里出错了：

![](https://github.com/sjf0115/ImageBucket/blob/main/Java/deep-into-generic-and-type-erasure-1.jpg?raw=true)
