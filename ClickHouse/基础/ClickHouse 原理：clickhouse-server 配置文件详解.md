...
```shell
root@67183d5fe6de:/etc/clickhouse-server# ll
total 100
drwxrwxrwx 1 clickhouse clickhouse  4096 Sep  5  2023 ./
drwxr-xr-x 1 root       root        4096 May  5 23:08 ../
drwxrwxrwx 1 clickhouse clickhouse  4096 Sep  5  2023 config.d/
-rw-rw-rw- 1 clickhouse clickhouse 73843 Sep  5  2023 config.xml
drwxrwxrwx 1 clickhouse clickhouse  4096 Sep  5  2023 users.d/
-rw-rw-rw- 1 clickhouse clickhouse  5370 Sep  5  2023 users.xml
```


注意:用户和查询级别设置是在“users.xml”文件中设置的。
如果您在这里意外指定了用户级别设置，则服务器将无法启动。
您可以将设置移动到“users.xml”文件中的正确位置
或者在这里添加<skip_check_for_incorrect_settings>1</skip_check_for_incorrect_settings>。

## 1. config.xml

### 1.1 logger

logger 是配置日志的路径、等级、日志滚动策略等元素。

- level：日志等级，默认为 `trace`。可以指定为 `none`、`fatal`、`critical`、`error`、`warning`、`notice`、`information`、`debug`、`trace`、`test`。
