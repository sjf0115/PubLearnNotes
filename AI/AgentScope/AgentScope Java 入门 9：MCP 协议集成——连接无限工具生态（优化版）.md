...
| 传输方式 | 通信机制 | 连接类型 | 典型场景 | 状态特性 |
|---------|---------|---------|---------|---------|
| **StdIO** | 标准输入输出流 | 启动子进程 | 本地 MCP Server（Node.js / Python 脚本） | 有状态 |
| **SSE** | HTTP Server-Sent Events | HTTP 长连接 | 远程 MCP Server（同局域网 / 云上） | 有状态 |
...
**原理**：以 `fsClient` 为例，AgentScope 启动子进程 `npx -y @modelcontextprotocol/server-filesystem /tmp`，通过 stdin / stdout 与 Server 交换 JSON-RPC 消息。
...
```java
// 单个参数
McpClientWrapper client1 = McpClientBuilder.create("mcp")
        .sseTransport("https://mcp.example.com/sse")
...
// 批量参数
McpClientWrapper client2 = McpClientBuilder.create("mcp")
        .streamableHttpTransport("https://mcp.example.com/http")
        .queryParams(Map.of("queryKey1", "queryValue1",
                            "queryKey2", "queryValue2"))
        .buildAsync()
        .block();

// 与 URL 中已有参数合并（额外参数优先）
McpClientWrapper client3 = McpClientBuilder.create("mcp")
        .sseTransport("https://mcp.example.com/sse?version=v1")
        .queryParam("queryKey", "queryValue")
        // 最终：?version=v1&queryKey=queryValue
        .buildAsync()
        .block();
```
...
| 适用场景 | 生产环境、Web 应用、高并发 | 脚本、测试、命令行工具 |
...
### 5.5 Elicitation 交互式补充

MCP 的 elicitation 功能允许在调用工具过程中，向用户发起**交互式信息收集请求**。当工具需要额外确认或补充参数时非常有用。

#### 5.5.1 异步 Elicitation
...
```java
List<String> enableTools  = List.of("read_file", "write_file",
...
> **优先级规则**：`disableTools` > `enableTools`。即使工具在白名单中，如果同时在黑名单中也会被排除。
...
toolkit.createToolGroup("filesystem", "Tools for operating system files", true);
