...
实体管理导航菜点击之后跳转管理页面，支持实体管理和实体类型管理两个Tab，详细参考上图UI。

配置实体点击 项目中心 → 实体管理 → 实体管理：
- 实体列表页，支持按照名称，类型查询，列表页展示实体名称、实体描述、实体类型、创建方式、操作(支持详情，编辑，删除操作)
- 点击 + 新增实体，完成实体名称/描述的填写，实体类型的下拉配置。

配置实体类型点击 项目中心 → 实体管理 → 实体类型管理：
- 实体类型列表页，支持按照实体类型名称查询，列表页展示实体类型、实体类型描述创建方式、操作(支持详情，编辑，删除操作)
- 点击 + 新增实体类型，完成实体类型名称/描述的填写。

已配置后端接口，请查阅后端API，注意参数命名
