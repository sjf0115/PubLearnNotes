...
我们想要回答的问题是：我们如何将收入下降精确到正确的维度及其要素?我们将问题重申如下。"找到一个布尔表达式，以维度和要素为单位，使归因于该表达式的收入下降最好地解释了总收入的下降。"当我们研究如何确定"最佳"时，请看如下表达式，它们可以解释收入下降50美元：

$$
Revenue\_Drop(DC == X) = \$47 \\
Revenue\_Drop(AD == A1 \vee AD == A3 \vee AD == A4) = \$51 \\
Revenue\_Drop(DT == Mobile \vee DT == Tablet) = \$49 \\
$$

例如，等式2表明，广告商表第1,3,4行预测收入与实际收入之差的总和为51美元，这与总收入下降50美元非常接近。一般来说，这样的表达式可以包含多个维度，例如 $Revenue\_Drop(DT == PC \vee DC == X)$，它指的是来自数据中心 X 的 PC 用户服务广告的收入下降。根据我们对广告系统大约一年的监测，通过人工研究和使用归因算法(将异常归咎于多个维度)，我们发现，多个维度共同导致一个根因的情况非常罕见。因此，为了简化阐述，在本文中，我们的讨论仅限于寻找一个只包含一个维度及其元素的布尔表达式来解释异常变化。

为了理解什么是"最佳"维度及其元素，我们研究了几种标准。考虑下面的斯特劳曼方法，它是我们最终问题陈述的动机：

- 斯特鲁曼:找到一个维度和它的一组元素，这些元素的收入下降至少是总收入下降的一个临界比例，$T_{EP}$，这样的维度最简洁。

我们将一组元素的解释力(EP)量化为指标变化它所解释的比例。我们将一组元素的简洁性(P)量化为表达式中元素的总数。因此，斯特鲁曼需要找到解释力至少为$T_{EP}$的表达式，并使用最少的元素数。奥卡姆剃刀理论表明，只要能在一定误差范围内解释下降，最简洁的集合就是最好的解释。通过这个论证，如果将$T_{EP}$设置为0.9，则三个表达式中最好的维度和元素集在表达式1中，因为仅数据中心X就可以解释总下降的94%。

然而，这种方法在多维度的情况下存在缺陷。虽然数据中心X的收入下降占总收入下降的94%，但请注意，预测和实际收入在两个数据中心X和y之间平均分布。数据中心X提供了预测收入的94%(100美元中的94美元)和实际收入(50美元中的47美元)。数据中心Y在这两个维度上都贡献了6%。相比之下，在设备类型维度中，PC设备类型贡献了预测收入的50%(100美元中有50美元)，但却贡献了实际收入的98%(50美元中有49美元)。移动和平板设备类型的贡献也相差很大，从预测收入的25%到实际收入的0%不等。广告主维度的贡献也各不相同，但没有设备类型的贡献大。

总体而言，设备类型维度不同元素的预测和实际收入之间的巨大变化令人惊讶的。因此，我们认为，与只使用表达的简洁性和解释力相比，那么惊喜更能说明问题。假设收入下降的根因是配置文件错误，导致手机和平板电脑上无法展示广告。虽然数据中心X仍然会出现巨大的收入下降，因为它提供了94%的跨设备展示广告，但实际的根因可以通过设备类型维度（手机和平板电脑元素）来更好地解释。换句话说，等式3中的表达式是最好的，尽管它不是最简洁的。

为了捕捉这一观察结果，我们的方法包含了一个与表达式相关的"惊喜"(S)的概念(第4节有精确的定义)。因此，对于任何指标，我们最终的收入调试问题陈述可以通过三个步骤来表述

- 对于一个维度，找到至少可以解释该维度变化的阈值分数($T_{EP}$)的所有元素集合(具有高解释力)。
- 在每个维度的所有这样的集合中，找出该维度中最简洁的集合。
- 在所有维度的所有这些集合中，找出在贡献变化方面最令人惊讶的一个集合。

在模拟示例中，当 $T_{EP} = 0.9 $ 时，第一步将把集合缩小到数据中心维度的 $\{X\}$ 元素集合，广告主维度的 $\{A1, A3, A4\}$ 元素集合，设备类型维度上的 $\{ Mobile, Tablet \}$ 和 $\{PC, Mobile, Tablet\}$。步骤2将每个维度的集合缩小到 $\{X\}$，$\{A1, A3, A4\}$ 和 $\{ Mobile, Tablet \}$。步骤3使用惊喜来选择设备类型维度，并将其集合$\{ Mobile, Tablet \}$作为下降的最佳解释。

我们的算法在度量中使用每个元素的变化阈值$T_{EEP}$来增加相似性的概念。我们不仅想要最小的元素集合，我们还想要那些对异常贡献最小的$T_{EEP}$部分的元素。我们在第4.4节中说明，在最坏的情况下，解决这个问题可能需要指数级的时间(在元素数量上)。因此，我们使用贪心方法近似地解决了这个问题。

## 3. 根因识别算法

首先我们先定义一些符号来定义解释力和惊喜。然后，我们介绍了根因识别算法。虽然基本指标和派生指标的算法是相同的，但计算派生指标的解释力和惊喜的方式更为复杂，将在第4节中单独讨论。

### 3.1 符号

表5汇总了本节中使用的重要术语及其符号。指标(例如，收入、搜索次数)使用 $M= \{ m_1,m_2,...,m_k\}$ 表示，维度集合(例如，广告商，数据中心)表示为 $D = \{D_1,D_2，…，D_n\}$。进一步，给定维度 $D_i$ 的元素集合表示为 $E_i = \{E_{i1},E_{i2}，…，E_{iC_i} \}$，其中 $C_i$ 是第 $i$ 个维度的基数，即 $D_i$ 维度。例如，$E_{21}$ 可以是广告商维度的的一个元素，可能是"Flowers123"。

对于每个感兴趣的指标 $m \in M$(包括基本指标和派生指标)以及每个元素 $E_{ij}$，我们可以获取预测值$F_{ij}$以及实际值 $A_{ij}$。需要注意的是，如前所述，这些值是某个时间间隔内的总和(例如，对于广告商维度，元素 Flower123，预测收入为100美元，实际收入为90美元)。对于收入或搜索次数等基本度量，指标的整体预测值 $F(m)$ 和整体实际值 $A(m)$ 在所有维度上都是一样的(例如，在前一节的示例中，预测收入为100美元，实际收入为50美元)。对于基本指标，总体指标值仅仅是各自维度元素的指标值的总和，但对于派生指标而言并非如此，因为它们不具有可加性(第4节)。因此，给定 $F(m)$ 和 $A(m)$，算法需要输出一个潜在的根因来解释两者之间的差异。为此，算法引入了解释力和惊喜的概念，定义如下。
...
$$
EP_{ij} = \frac{A_{ij}(m) - F_{ij}}{A(m) - F(m)}
$$

例如，某一时刻的总搜索量从预测值100万偏离到80万，而某一数据中心 $DC1$ 同一时刻的搜索量从预测值50万偏离到40万，那么元素 $DC1$ 的解释能力为 (0.4-0.5)/(0.8-1)= 50%。请注意，如果元素的变化与整体变化的方向相反，则元素的解释力可以超过100%甚至是负的。但是，任何维度的所有要素的解释力之和应达到100%。因此，解释力充分解释了总体度量的变化。

> 解释力即元素值变化占整体变化的比值

### 3.3 惊喜度

正如在第2节的示例中所讨论的那样，与未发生变化的维度(例如，数据中心)相比，分布变化较大的维度(例如，设备类型)更有可能是根因。我们现在正式定义了一个惊喜度指标来捕捉这个概念。

对于每个元素 $E_{ij}$, 让 $p_{ij}(m)$ 为预测或先验概率值，其值计算如下所示：
...
直观地说，如果后验概率分布与先验概率分布显著不同，对于给定维度的新观察结果就是令人惊讶的。两个概率分布 P 和 Q 之间的差异可以通过相对熵 Kullback-Leibler (KL)散度来描述。不过，在这里使用 KL 散度有两个问题。首先，KL 散度不是对称的。其次，只有当 $q_i=0$(只有当$p_i=0$)才定义 KL 散度，而在我们这里不成立(例如，广告商暂停他的广告活动)。因此，我们不能使用 KL 散度，而是使用一种称为 Jensen-Shannon (JS) 散度的指标来计算惊喜度，其定义为：

$$
D_{JS}(P,Q) = 0.5(\sum{ip_{i}}\log\frac{2p_i}{p_i + q_i} + \sum{iq_{i}}\log\frac{2q_i}{p_i + q_i})
$$

观察到 $D_{JS}(P,Q)$ 是对称的，即使$q_i=0$和/或 $pi=0$ 时也是有限的。此外，$0 \le D_{JS}(P,Q) \le 1$，其中 0 表示 P 和 Q 的分布没有变化，值越大表示差异越大。因此，为了计算$E_{ij}$的惊喜度 $S_{ij}$，我们使用 $p=p_{ij}(m)$ 和 $q=q_{ij}(m)$ 进行计算：
...
根因识别算法试图使用上面的解释力和惊喜度来解决第2节中的优化问题。需要注意的是，在最坏的情况下获得问题的最优解将花费指数级的时间。这可以通过一个简单的例子来说明：考虑一个大小为 n 的集合，其中每个元素都具有相同的解释力，我们需要集合的 $n/2$ 个元素来解释 $T_{EP}$。在这种情况下，基数为 $n/2$ 的每个可能的子集都是可能是最小的(简洁)，并且具有$T_{EP}$的解释力。因此，我们必须比较所有这些子集(其数量与n成指数关系)的惊喜值，以找到具有最大惊喜值的子集，即最优解。

我们的算法(图2)使用了以下贪心启发式方法，而不是枚举具有至少相当于$T_{EP}$解释力的各种最小基数子集。在每个维度中，在计算所有元素的惊喜度(第1-5行)之后，首先按照惊喜度从高到低的顺序对元素进行排序(第8行)。然后只要该元素本身至少解释了总异常变化的 $T_{EEP}$(第12 - 15行)就将每个元素添加到候选集合中。参数$T_{EEP}$帮助控制集合的基数(奥卡姆剃刀)。例如，如果 $T_{EEP}$ 为10%， $T_{EP}$ 为 67%，那么我们最多可以有7个元素可以解释异常变化。

此外，通过按惊喜度降序排列，我们贪婪地寻求候选集的惊喜度的最大化。该算法每个维度最多添加一个候选集(第16-19行)，只要候选集能够解释异常变化的大多数($T_{EP}$})的(解释力)，算法就会在每个维度上最多添加一个候选集(第16-19行)。最后，算法根据惊喜度对各种候选集进行排序，并返回最令人惊喜的前三个候选集作为潜在的根因候选集(第21-22行)。

## 4. 派生指标

派生指标是由故障排除人员跟踪的基本指标的函数，因为与简单地跟踪基本指标相比，派生指标能揭示更多的信息。在本节中，我们将讨论如何计算派生指标的解释力和惊喜度。
...
我们假设，如果某项数据与预期值相差至少20%，就会出现异常。需要注意的是，整体收入下降了10%，而点击次数却上升了16%，两者都没有超过异常阈值。相应的每次点击成本值如表8所示，并且使用相同的20%阈值，每次点击成本(下降22.5%)可以标记为异常。因此，可以看到派生的测量方法可以用于显示异常，而这些异常仅通过检查基本测量方法无法显示出来。我们在第6节中定量地证实了这一点。



派生指标归因问题如下：如何将每次点击总成本从0.2(预期)下降到0.155(实际)归因于每个广告商?    如果检查表8中每个广告主的每次点击成本，我们会发现广告主A1、A2、A4的每次点击成本没有变化，而广告主 A3 的每次点击成本增加了。因此，乍一看，似乎没有一个广告商应该为整体下降负责，但肯定至少有一个或多个广告商必须负责！在这种情况下，我们如何为这些广告商分配每次点击成本变化的解释力值呢?



对基本指标的研究确实有助于揭示更多的信息。例如，尽管A1的每次点击成本保持不变，但与预期收入和点击量相比，A1的下降幅度为5倍。假设A1的每次点击成本(0.5)高于总体值(0.2)，5倍的降低意味着A1确实降低了每次点击的总体成本。如果我们使用a1的实际值，但假设其余的广告商都提供了各自的预测值，那么每次点击的总成本就会下降到60/420 = 0.143，影响为-29%，这一事实可以进一步证实a1解释了总体衍生平均值的下降。
