...
```
参数的 ObjectInspector:在 PARTIAL1 和 COMPLETE 模式下，输入参数是原始数据。在 PARTIAL2 和 FINAL 模式中，输入参数是部分聚合结果(在这种情况下，数组将始终只有一个元素)。

在 PARTIAL1 和 PARTIAL2 模式下，terminatePartial() 返回值的 ObjectInspector 为 StandardStructObjectInspector。在 FINAL 和 COMPLETE 模式下，terminate() 返回值的 ObjectInspector 为 WritableDoubleObjectInspector。
...
```
terminatePartial 方法一般在 Map 或者 Combine 阶段结束时调用，用来将数据以持久化方式传输到 Reduce 进行处理。只支持 Java 原始数据类型、数组、原生包装器(例如，Double)、Hadoop Writables、Lists 或者 Map 类型。不能使用我们自己自定义类(即使实现了 java.io.Serializable)，否则可能会得到奇怪的错误或(可能更糟)错误的结果。

#### 4.2.6 merge

merge 方法将 terminatePartial 返回的聚合结果 partial 合并到当前聚合结果 agg 中。我们通过 terminatePartial 方法知道传输到 merge 方法的是一个 Object 数组，因此我们需要通过 StandardStructObjectInspector 解析出 partial 数组元素值：
```java
public void merge(AggregationBuffer agg, Object partial) throws HiveException {
    if (partial == null) {
        return;
...
terminate 方法根据 AggregationBuffer 中临时存储的值计算生成 DoubleWritable 类型的平均值 Object 返回：
```java
private DoubleWritable result;
public Object terminate(AggregationBuffer agg) throws HiveException {
    AverageAggBuffer buffer = (AverageAggBuffer) agg;
...
create temporary function simple_avg as 'com.hive.example.udf.AbstractGenericAverageUDAF';

SELECT simple_avg(a) FROM (
  SELECT 10 AS a
  UNION ALL
  SELECT 30 AS a
  UNION ALL
...
https://www.jianshu.com/p/4580052a94e6
