...
Dify 作为一个开源的 LLM 应用开发平台，通过其强大的知识库功能，完美地搭建了私有数据与大模型之间的桥梁。本文将手把手带你深入 Dify 知识库的构建全过程，从基础概念到实践，助你打造一个“懂业务”的 AI 问答专家。

## 1. 简介

在开始动手之前，我们先要理解知识库的核心价值。在 Dify 中，你可以将自有数据作为「知识」集成到 AI 应用中。通过为大语言模型 LLM 提供特定领域的上下文信息，知识能够让 LLM 的回复更加准确、相关，并显著减少幻觉。Dify 知识库本质上是一个检索增强 RAG 生成的最佳实践工具。其核心在于：LLM 不再只依赖预训练的公开数据，还会将你的自定义知识作为额外的事实来源：
...
![](img-dify-knowledge-14.png)

## 4. 如何在 AI 问答助手中应用知识库

### 4.1 创建 AI 问答助手

创建一个通用型 AI 问答助手。依次单击工作室>创建空白应用，并为 AI 问答助手输入名称和描述，其他参数保持默认即可。

![](img-dify-knowledge-16.png)

现在您可以在页面输入问题以查看目前 AI 问答助手配置的最新效果。由于下图示例中调用的是一个通用型聊天机器人，所以只能进行简单对话，暂时还无法详细回答 Dify 相关的专业问题。

![](img-dify-knowledge-17.png)

### 4.2 定制 AI 问答助手

通过前面的操作，您已经拥有了一个可以对话的 AI 问答助手。但是如果您想让 AI 问答助手像专业人士一样，更加精准且专业地回答 Dify 技术相关的问题，您还需要为 AI 问答助手配置专属语料知识库。为简化操作步骤，本示例已经为您整理好语料文件 `dify_doc.md`，您只需按照上述介绍创建知识库的步骤进行操作，即可完成创建和上传专属知识库。

![](img-dify-knowledge-18.png)

为已创建的 AI 助手配置提示词并添加上下文知识库。拷贝以下内容至提示词中。提示词用于对AI的回复做出指令和约束，可以提升AI问答助手的回答表现和结果准确性:
```
...
```
添加上下文知识库：单击上下文区域的添加，根据页面提示为 AI 问答助手配置的专属知识库，使 AI 问答助手获得精准且专业回答问题的能力。最后在页面右上角单击发布>更新手动保存配置信息，使 Dify 服务的配置生效。

![](img-dify-knowledge-19.png)

相较于上文的通用型聊天机器人，配置了专属知识库的 AI 助手通过集成相关领域的知识库，能够提供更加专业、准确的信息和建议。

![](img-dify-knowledge-20.png)
