...
先将文件上传保存到OSS，文件上传成功之后提示：文件上传成功，请先保存群组，在群组列表中查看计算结果。第一次保存时会将文件中的用户保存到群组对应的表中。

状态：计算中

文件上传创建群组的 group_rule：
- uuid_file_key：远程存储的 MinIO 文件地址
- file_name：原始上传文件名称

```json
{
    "file_key": "upload_group/20251130/fd5cd375-c5d6-4268-af8b-cf628107f201.csv",
    "file_list": [
        {
            "name": "group_template.csv"
        }
    ]
}
```
