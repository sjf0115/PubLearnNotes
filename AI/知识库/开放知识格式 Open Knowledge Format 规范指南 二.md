...
- **绝对路径（知识包相对路径）**：以 `/` 开头，相对于知识包根目录进行解析。这是 **推荐** 形式，因为当文档在其子目录内被移动时，链接依然保持稳定。
  ```markdown
  关联键请参见 [customers 表](/tables/customers.md)。
  ```
- **相对路径**：一个标准的 Markdown 相对路径。
  ```markdown
  请参见[相邻概念](./other.md)。
  ```

从概念 A 指向概念 B 的链接，即声明了一种关系。具体关系类型，如父子关系、引用、关联或依赖，由链接周围的文本表达，而非链接本身。构建图视图的消费者通常将所有链接视为无类型关系的有向边。消费者必须容忍断开的链接：如果链接目标在知识包中不存在，并不表示该链接格式错误，它可能仅仅代表了尚未编写的知识。

### 6.2 路径类型字段

多个字段使用路径或 URI 来命名资源：`resource`、`sources[].resource`、`computation`、`executor.resource` 和 `attester.resource`（参见第 10 节）。但 `sources[].resource` 也可以是一个范围描述符（参见第 5.1 节），在这种情况下它不是一个路径。每个路径值字段接受：
- 绝对 URL，例如 `https://...`；
- 以 `/` 开头的知识包相对路径；
...
按照约定，`references/` 子目录用于将外部材料、运行说明或代码作为知识包内的一等概念。来源（Sources）、执行器（Executors）和证明者（Attesters）通常指向该目录（例如 references/attesters/revenue.py）。这是一个命名约定，而非强制要求。

---

## 7. 参与者约定

记录身份标识的字段（`generated.by`、`verified[].by`）使用统一的参与者约定：
...
对信任进行分类的消费者（参见 5.3 节）会依据 `human:` 前缀作为关键判断依据，因此生产者对人工编写或人工确认的内容必须使用该前缀。

---

## 8. 索引文件

`index.md` 文件可以出现在任何目录中，包括知识包根目录。它枚举该目录下的内容，以支持渐进式披露：让人或智能体在打开单个文档之前，先了解当前目录下有哪些可用内容。

> 在没有 index.md 的情况下，如果你想了解一个目录里有什么，要么打开文件系统逐一点击，要么让 AI 扫描所有文件并生成报告，这在大型知识库中成本极高。

索引文件不包含元数据块，只有一个例外：知识包根目录的 `index.md` 可以包含 `okf_version` 键（参见第 12 节）。正文使用一个或多个章节，每个章节通过标题对概念分组：
```markdown
# 章节或分组标题
...
* [子目录](subdir/) - 子目录的简短说明
```
`index.md` 文件中的条目应该包含被链接概念元数据块中的 description（描述）。生产者可以自动生成 `index.md`；消费者也可以在缺少 `index.md` 时动态合成一个（类似的视图）。

---

## 9. 日志文件

`log.md` 文件可以出现在层级结构的任何级别，用于记录该范围内的变更历史。其格式是按日期分组的扁平条目列表，最新内容位于最前：
```markdown
# 目录更新日志
...
日期标题必须采用 ISO 8601 标准中的 `YYYY-MM-DD` 格式。日志条目使用自然语言（自由文本）；开头的加粗词（`**更新**`、`**创建**`、`**弃用**`）是一种约定俗成的写法，而不是强制要求。

---

## 10. 已证明计算概念

已证明计算概念不仅承载了一个值的含义，还承载了计算该数值的受认可方式，使消费者能够确认智能体运行的是被认可的计算，而不是自行编造逻辑。来源追溯（参考第5节）回答的是‘这项声明来自哪里’；证明回答的是‘这个数字是否按照我们规定的方式生成’。OKF 记录计算本身及其检查手段，但它本身并不执行任何计算。

### 10.1 一项计算就是一个独立概念

一个受认可的计算是一个 `type: Attested Computation` 的独立概念。需要该值的概念（如 `Metric`、`BigQuery Table`）通过普通 Markdown 链接指向它（参考第6节）。将计算建模为独立概念有三个原因：
- **运行时 `runtime` 定义了参数的含义。** 参数可以是 SQL 绑定变量、dbt 变量或 Python 参数，具体取决于运行时。将运行时和参数放在同一个元数据块中，使得绑定语义不言自明。
- **一个计算，多个消费者。** 同一个计算可以同时支撑一个指标（Metric）、一个仪表板概念（Dashboard concept）和一个报表（Report）；作为一个独立概念，它被引用一次，便可复用多次。
- **信任状态是针对每个计算的。** `verified`、`stale_after` 和单个 `attester` 描述的是同一项计算。收入、利润和利润率需要分别验证和证明，因此应建模为三个概念，而不是同一元数据块中的三个条目。

### 10.2 契约字段

契约就是概念的顶层前置元数据。除来源追溯、信任和生命周期字段组外，已证明计算概念还包含：
- `runtime`：此类型的必填字段。这是唯一一个指明如何运行计算的字段，同时也定义了执行器（executor）和证明者（attester）如何解释它，以及参数（parameters）的含义。示例值：`bigquery`、`postgres`、`dbt`、`python`、`Looker`。
- `parameters`：智能体可以可以填充的、带有类型的命名占位符（holes）列表。每个条目为 `{ name, type, required }`。绑定语义取决于 `runtime`。
- `computation`：可选。指向保存计算内容的文件的路径，用于代替正文中的内联围栏代码块（见第 10.3 节）。缺失时，正文 `# Computation` 下的代码块就是计算内容。
- `executor`：说明计算如何运行。`resource` 指向运行说明或代码，由运行程序（智能体或确定性消费者代码）遵循；`receipt` 声明一次运行必须返回的字段，即证明者（attester）检查的证据，例如 BigQuery `job_id` 和作业实际执行的 SQL。
...
可以通过以下两种方式之一提供计算：
- **内联**：在正文 `# Computation` 标题下放置一个单独的围栏代码块。最适合简短、且与合约（元数据）一同审核的计算逻辑。
- **文件**：将 `computation` 设置为路径（见第6.2节），并省略正文中的代码块围栏。最适合较长或由工具生成的计算逻辑，或者已经作为实际文件保存并与非 OKF 工具共享的情况。
...
智能体只能为已声明的参数提供值，它绝不能编写或编辑计算逻辑。将 `computation` 与参数值绑定为可执行工件是消费者的职责；而证明者（Attester）会独立地重新推导出相同的绑定，以与实际运行的内容进行比较。由于比较对象是执行凭证携带的展开、编译后制品（`executed_sql`、`compiled_sql`），重写查询、替换计算文件或修改依赖项都会导致检查失败。只开放强类型参数的接口，才能把“是否运行了受认可计算”转化为机械比较，而非主观判断。

### 10.4 使用计算的概念

一份文档通常不会只涉及一项计算。一份讨论收入（Revenue）、利润（Profit）和毛利率（Margin）的损益表概览（income-statement overview），仍然可以作为一个可读的独立概念，同时为每个数值链接一项已证明计算：
```markdown
---
type: Metric
title: 收入
...
由于每个计算都是独立的概念，收入（Revenue）可以保持新鲜（有效），而利润（Profit）可能已经超过其 stale_after（过期时间），并且每个计算都针对其自身的运行进行证明（attests on its own run）。将它们放在一起（共置）是一个目录层面的选择（例如在 `computations/` 文件夹中放置一个 `index.md`），而不是一个 frontmatter 层面的选择。

### 10.5 消费者如何使用已证明计算

本小节仅提供资料，不属于规范性要求。以下运行时工件（runtime artifacts）不存储在知识包中：
- **发现**：通过 `type: Attested Computation` 发现计算。这是一个可以被提取到 `index.md` 中的元数据块信号；消费者可以直接访问它，或通过跟随使用它的概念中的链接来访问。
- **加载**：从元数据块加载契约，从正文或 `computation` 指定的文件加载计算。
- **参数化**：智能体为已声明参数提供值。
- **执行**：执行器运行绑定后的计算，并返回符合 `executor.receipt` 结构的执行凭证。
- **证明**：消费者使用证明器检查执行凭证。证明器确认来源真实性（实际运行的计算等于 `computation` 与声明参数绑定后的结果，而不是智能体编写的 SQL）和结果保真度（显示值与执行凭证中的权威来源一致；应按作业 ID 重新读取，而不是直接采用智能体文本中的数值）。
- **准入控制**：证明失败时拒绝展示；当 `now >= stale_after` 时发出警告或拒绝展示。成功时应呈现判定结果，例如提供作业日志链接，让信任状态清晰可见。
...
知识包满足以下条件时，即符合 OKF v0.2 规范：
- 目录树中每个非保留的 `.md` 文件都包含可解析的 YAML 元数据块。
- 每个元数据块都包含一个非空 `type` 字段。
- 保留文件 `index.md`、`log.md` 存在时，需要分别遵循第 8 节和第 9 节的结构。

> 整个规范只有三条硬性要求，这是 OKF 保持极简主义的基石

当信任、生命周期、来源追溯或计算系列字段存在时，生产者应遵循第 5 节至第 10 节内容，而消费者则：
- **必须** 将 `verified` 简写模式(不带 `-` 破折号)视为单元素列表（第 5.2 节）。
- **不得** 因缺少任何可选字段而拒绝概念（第 5.3 节）。
- **应** 仅根据本文规定的字段推导信任层级和过时状态；并且应该（SHOULD）展示（而非静默丢弃）失败的证明（第 10.5 节）。
