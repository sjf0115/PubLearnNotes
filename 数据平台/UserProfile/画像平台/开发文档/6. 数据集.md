...
在数据集列表右侧点击新建数据集按钮“+”，在弹出的弹框中选择数据集的创建方式。当前提供「标签数据集、行为数据集、统计数据集、特征数据集」4种数据集的创建方式：
标签数据集：标签数据集记录实体(用户)的一系列特征属性。通过为字段配置标签别名的方式给实体(用户)打标签，在使用标签时，实质上是在使用对应的字段。。
行为数据集：行为数据集记录用户的浏览、加购、收藏、购买等行为。
统计数据集：统计数据集记录实体(用户)执行相关操作的统计数据。
特征数据集：特征数据集是算法特征库。

数据集列表页：
- 数据集名称、数据集描述、数据集状态、数据集类型、负责人、数据更新时间、创建时间、数据源名称、数据表名、操作(查看、编辑、启用/禁用、删除、绑定标签、调度、历史)
...
优化创建标签数据集功能：实体字段 和 实体标识 放在一行，先展示实体标识后实体字段；字段列表中添加一列关联标签列，用于配置每个字段可以关联的标签。标签可以下拉选择用户选择的实体标识对应的所有标签，可以通过给 /label/list 接口传递 entity_identifier_id 参数获取对应实体标识下的所有标签。如果用户没有先选择实体标识，无法下拉选择标签。


请你结合标签数据集的创建功能，再完善标签数据集的查看与编辑功能。点击查看和编辑功能跳转到查看和编辑详情页。查看功能尽可能多的展示标签数据集的详细信息。
...
```json
{
  "dataset_name": "用户属性标签数据集",
  "dataset_type": 1,
  "dataset_desc": "用户属性标签数据集",
  "datasource_id": "0500000000000002",
  "table_name": "dws_user_profile_attr_label_td",
  "partition_field": "dt",
  "partition_format": "yyyyMMdd",
  "entity_id": "0219740078368128",
